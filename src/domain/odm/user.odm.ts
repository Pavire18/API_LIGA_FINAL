...
const getAllUsers = async (page: number, limit: number): Promise<any> => {
  return await User.find()
    .limit(limit)
    .skip((page - 1) * limit).populate("team");
};
...
const getUserByEmailWithPassword = async (email: string): Promise<Document<IUser> | null> => {
  const user: Document<IUser> | null = await User.findOne({ email }).populate("team").select("+password") as any;
  return user;
};

const getUserByName = async (name: string): Promise<Document<IUser>[]> => {
  return await User.find({ firstName: new RegExp("^" + name.toLowerCase(), "i") }).populate("team");
};
